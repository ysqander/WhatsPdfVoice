...
export class MemStorage implements IStorage {
  ...
  async uploadMediaToR2(
    ...
    try {
      ...
      console.log(
        `[STORAGE UPLOAD] directory=${directory}, originalName=${originalName}, key=${key}`
      )
      ...
      console.log(
        `[STORAGE] Uploaded media file to R2: id=${mediaFile.id}, key=${key}, url=${url}`
      )
      return mediaFile
    } catch (error) {
  ...
  async getMediaUrl(mediaId: string): Promise<string> {
    ...
    try {
      ...
      if (mediaProxy) {
        ...
        console.log(
          `[STORAGE FETCH] mediaId=${mediaId}, key=${mediaProxy.r2Key}, url=${mediaProxy.r2Url}`
        )
    ...
    console.log(
      `[STORAGE FETCH] (memory) mediaId=${mediaId}, key=${mediaFile.key}, url=${url}`
    )
    return url
  }
  ...
  async deleteMedia(mediaId: string): Promise<boolean> {
    ...
    if (mediaFile.messageId) {
      ...
      if (message) {
        message.mediaUrl = undefined // Set to undefined, not null
