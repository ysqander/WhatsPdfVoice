    ...
    processFile: async (req: Request, res: Response) => {
        try {
            ...
            process.nextTick(async () => {
                ...
                try {
                    console.log(
                        `Waiting for SSE connection for clientId: ${clientId}`,
                    );
                    await waitForClient;
                    console.log(
                    ...
                    if (!client) {
                        ...
                        if (req.file && fs.existsSync(req.file.path)) {
                            try {
                                ...
                                console.error(
                                    "Error cleaning up file on client disconnect:",
                                    e,
                                );
                            }
                        }
                    ...
                    client.write(
                        `data: ${JSON.stringify({ progress: 5, step: ProcessingStep.EXTRACT_ZIP })}\n\n`,
                    );
                    ...
                    client.write(
                        `data: ${JSON.stringify({ progress: 20, step: ProcessingStep.EXTRACT_ZIP })}\n\n`,
                    );
                    ...
                    client.write(
                        `data: ${JSON.stringify({ progress: 30, step: ProcessingStep.PARSE_MESSAGES })}\n\n`,
                    );
                    ...
                    console.log(
                        `Total media size calculated: ${totalMediaSize} bytes (${(totalMediaSize / (1024 * 1024)).toFixed(2)}MB)`,
                    );
                    ...
                    if (requiresPayment) {
                        ...
                        try {
                            ...
                            const bundle =
                                await paymentService.createPaymentBundle(
                                    savedChatExport.id!,
                                    chatData.messages.length,
                                    totalMediaSize,
                            ...
                            for (const message of messagesToProcessMedia) {
                                // mediaUrl from parser should be like /media/filename.ext
                                const localFilename = path.basename(
                                    message.mediaUrl!,
                                );
                                ...
                                const sourceMediaPath = fs.existsSync(
                                    localMediaPath,
                                )
                                    ? localMediaPath
                                ...
                                if (sourceMediaPath && message.id) {
                                    ...
                                    try {
                                        ...
                                        await storage.updateMessageProxyUrl(
                                            message.id,
                                            proxyUrl,
                                        );
                                        console.log(
                            ...
                            const finalMessages =
                                await storage.getMessagesByChatExportId(
                                    savedChatExport.id!,
                                );
                            console.log(
                            ...
                            throw new Error(
                                `Failed during payment setup: ${error instanceof Error ? error.message : error}`,
                            );
                        }
                    }
                    ...
                    console.log(
                        "Processing as free tier or payment failed/not required.",
                    );
                    ...
                    for (const message of chatData.messages) {
                        try {
                            ...
                            console.error(
                                `Error saving message (Free Tier Path): ${err}`,
                            );
                        }
                    }
                    chatData.messages = finalSavedMessages; // Use messages with IDs
                    console.log(
                    ...
                    client.write(
                        `data: ${JSON.stringify({ progress: 60, step: ProcessingStep.CONVERT_VOICE })}\n\n`,
                    ); // Keep step for consistency

                    const r2Connected = await testR2Connection().catch(
                        (err) => {
                    ...
                    client.write(
                        `data: ${JSON.stringify({ progress: 80, step: ProcessingStep.GENERATE_PDF })}\n\n`,
                    );
                    ...
                    if (r2Connected) {
                        try {
                            console.log(
                                `Uploading final generated PDF: ${pdfResultPath}`,
                            );
                        ...
                        finalPdfUrl = `/api/whatsapp/pdf/${path.basename(pdfResultPath)}`;
                        console.log(
                            "Using local PDF URL (R2 not connected):",
                            finalPdfUrl,
                        );
                    }
                    ...
                    if (clientRes) {
                        try {
                            clientRes.write(
                                `data: ${JSON.stringify({ error: `Processing failed: ${error instanceof Error ? error.message : "Unknown error"}` })}\n\n`,
                            );
                            clientRes.end();
                        } catch (sseError) {
                            console.error(
                                "Error sending error message via SSE:",
                                sseError,
                            );
                        }
                        clients.delete(clientId);
                    ...
                    console.log(
                        `Background processing finished for ${clientId}. Elapsed time: ${Date.now() - processingStartTime}ms`,
                    );
                    ...
                    if (extractDir && fs.existsSync(extractDir)) {
                        try {
                            ...
                            console.log(
                                "Cleaned up extraction directory:",
                                extractDir,
                            );
                        } catch (e) {
                            console.error(
                                "Error cleaning up extraction directory:",
                                e,
                            );
                        }
                    }
    ...
    downloadPdf: async (req: Request, res: Response) => {
        try {
            ...
            if (!chatExport) {
                ...
                return res
                    .status(404)
                    .json({ message: "Chat export not found" });
            }
            ...
            if (!chatExport.pdfUrl) {
                ...
                const pdfFile = mediaFiles.find(
                    (file) =>
                        ...
                        (file.originalName === "MAIN_GENERATED_PDF" ||
                            (file.fileHash &&
                                file.fileHash.startsWith("MAIN_PDF_"))),
                );
                ...
                if (pdfFile) {
                    ...
                    return res
                        ...
                        .json({
                            message:
                                "PDF not generated or linked for this chat",
                        });
                }
            }

            console.log(
                `Attempting to serve PDF from URL: ${chatExport.pdfUrl}`,
            );
            ...
            if (chatExport.pdfUrl.startsWith("/api/media/proxy/")) {
                ...
                console.log(
                    `Redirecting to PDF proxy URL: ${chatExport.pdfUrl}`,
                );
                ...
                res.setHeader(
                    "Content-Disposition",
                    `inline; filename="WhatsApp_Chat_${format(new Date(), "yyyyMMdd")}.pdf"`,
                );
                ...
                console.error(
                    `Unrecognized PDF URL format: ${chatExport.pdfUrl}`,
                );
                return res
                    .status(500)
        ...
        try {
            ...
            try {
                ...
                return res
                    .status(500)
                    .json({
                        message: `Failed to retrieve PDF: ${fetchError instanceof Error ? fetchError.message : fetchError}`,
                    });
            }
            ...
            [attachmentsDir, audioDir, imageDir, pdfDir, otherDir].forEach(
            ...
            const downloadPromises = mediaFiles
                ...
                .map(async (file) => {
                    ...
                    const fileOriginalName =
                        file.originalName ||
                        path.basename(file.key || "unknown");
                    ...
                    try {
                        ...
                        if (!fetchUrl)
                            throw new Error(
                                "Could not get valid URL for media file",
                            );
            ...
            res.setHeader(
                "Content-Disposition",
                `attachment; filename="WhatsApp_Evidence_${chatId}_${format(new Date(), "yyyyMMdd")}.zip"`,
            );
            ...
            const manifest = {
                ...
                messageCount: chatExport.messages?.length || messages.length, // Use chatExport if available, else messages var
                mediaCount: {
                    ...
                    total: mediaFiles.filter(
                        (f) =>
                            !(
                                ...
                                (f.fileHash &&
                                    f.fileHash.startsWith("MAIN_PDF_"))
                            ),
                    ).length,
...
`;
            ...
            setTimeout(() => {
                try {
                    ...
                    console.error(
                        `Error cleaning up temp files for chat ${chatId}:`,
                        err,
                    );
                }
