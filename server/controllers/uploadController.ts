...
export const uploadController = {
    ...
    try {
      ...
      process.nextTick(async () => {
        try {
          ...
          if (!r2Connected) {
            console.warn(
              "R2 connection failed, using local storage for media files",
            );
          } else {
            console.log(
            ...
            const messages = await storage.getMessagesByChatExportId(
              savedChatExport.id!,
            );
            ...
            console.log(
              `Found ${mediaMessages.length} media messages to upload to R2`,
            );
            ...
            for (const message of voiceMessages) {
              try {
                ...
                if (fs.existsSync(mediaPath)) {
                  ...
                  await storage.updateMessageMediaUrl(
                    message.id,
                    mediaFile.key,
                    mediaFile.url!,
                  );
                  console.log(
                    `Uploaded voice message to R2, key: ${mediaFile.key}`,
                  );
                } else {
                ...
                console.error(
                  `Error uploading voice message for message ${message.id}:`,
                  error,
                );
              }
            }
            ...
            const otherMediaMessages = mediaMessages.filter(
              (msg) => msg.type !== "voice",
            );
            console.log(
              `Processing ${otherMediaMessages.length} other media messages`,
            );
            for (const message of otherMediaMessages) {
              try {
                ...
                if (fs.existsSync(mediaPath)) {
                  ...
                  console.log(
                    `Uploaded ${message.type} to R2, key: ${mediaFile.key}`,
                  );
                }
              } catch (error) {
                console.error(
                  `Error uploading media file for message ${message.id}:`,
                  error,
                );
          ...
          const updatedMessages = await storage.getMessagesByChatExportId(
            savedChatExport.id!,
          );
          ...
          console.log(
            "PDF URL saved to storage for chat export:",
            savedChatExport.id,
          );
  ...
  downloadPdf: async (req: Request, res: Response) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="WhatsApp_Chat_${new Date().toISOString().slice(0, 10)}.pdf"`,
      );
  ...
  downloadEvidenceZip: async (req: Request, res: Response) => {
    try {
      ...
      if (chatExport.pdfUrl.startsWith("http")) {
        ...
        if (pdfFile) {
          ...
          try {
            ...
            return res
              .status(500)
              .json({ message: "Failed to download PDF from cloud storage" });
          }
        } else {
          return res
            .status(404)
            .json({ message: "PDF not found in cloud storage" });
        }
      } else {
        ...
        if (!fs.existsSync(pdfPath)) {
          return res
            .status(404)
            .json({ message: "PDF file not found in local storage" });
        }
      }
      ...
      console.log(
        `Downloaded ${successfulDownloads.length} of ${downloadResults.length} media files for evidence zip`,
      );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="WhatsApp_Evidence_${chatId}_${new Date().toISOString().slice(0, 10)}.zip"`,
      );
      ...
      const manifest = {
        ...
        mediaCount: {
          total:
            mediaFiles.length -
            (mediaFiles.find((f) => f.type === "pdf") ? 1 : 0),
