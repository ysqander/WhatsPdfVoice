...
router.get('/api/media/proxy/:mediaId', async (req: Request, res: Response) => {
  try {
    ...
    if (!mediaProxy) {
      console.error(`[MEDIA PROXY] Media file not found with ID: ${mediaId}`)
      return res.status(404).json({ error: 'Media file not found' })
    }

    console.log(
      `[MEDIA PROXY] Found record: mediaId=${mediaId}, r2Key=${mediaProxy.r2Key}, r2Url=${mediaProxy.r2Url}`
    )
    ...
    console.log(`[MEDIA PROXY] Redirecting to R2 URL for mediaId=${mediaId}`)
    return res.redirect(mediaProxy.r2Url)
  } catch (error) {
    console.error('[MEDIA PROXY] Failed to proxy media request:', error)
