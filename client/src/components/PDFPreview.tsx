...
export default function PDFPreview({ 
  ...
  const [groupedMessages, setGroupedMessages] = useState<{ [date: string]: Message[] }>({});
  
  // Group messages by date when chatData changes
  useEffect(() => {
    if (chatData && chatData.messages) {
      const grouped = chatData.messages.reduce<{ [date: string]: Message[] }>((acc, message) => {
  ...
  return (
    ...
    >
      ...
      <div className="border-b-2 border-gray-200 pb-4 mb-6">
        <h1 className="text-xl font-bold text-center uppercase mb-1">CHAT TRANSCRIPT EVIDENCE</h1>
        <h2 className="text-lg text-center mb-4">WhatsApp Conversation</h2>
        
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <p><span className="font-semibold">Case Reference:</span> WA-{format(new Date(), 'yyyyMMdd-HHmm')}</p>
            <p><span className="font-semibold">Generated On:</span> {format(new Date(), 'dd MMM yyyy, HH:mm')}</p>
          </div>
          <div>
            <p><span className="font-semibold">File SHA-256:</span> {chatData?.fileHash ? chatData.fileHash.substring(0, 6) + '...' : '---'}</p>
            <p>
              <span className="font-semibold">Participants:</span> {chatData?.participants?.join(', ') || '---'}
            </p>
          </div>
        </div>
      </div>
              ...
              <div className="flex flex-col" key={`${dateIndex}-${msgIndex}`}>
                <div className="flex items-start mb-1">
                  ...
                  <span className={`font-semibold text-sm ${
                    message.sender === chatData?.participants?.[0] ? 'text-primary' : 'text-secondary'
                  }`}>
                    {message.sender}
                  </span>
                </div>
                <div className={`rounded-lg p-3 ml-6 ${
                  message.sender === chatData?.participants?.[0] ? 'bg-gray-100' : 'bg-blue-50'
                }`}>
                  {message.type === 'voice' ? (
                    <AudioPlayer audioUrl={message.content} duration={message.duration || 0} />
                  ) : (
